...
def record_user_session(url, domain, har_filename, har_filename_raw):
    """
    ...
    print(
        f"{bcolors.OKBLUE}[INFO]: Opening Firefox with Selenium Wire to capture traffic...{bcolors.ENDC}"
    )
    print(
        f"{bcolors.OKBLUE}[INFO]: If the Firefox browser does not open automatically, please exit and rerun Tampy.{bcolors.ENDC}"
    )
    ...
    input(
        f"{bcolors.BOLD}Press Enter when you have finished the interaction...{bcolors.ENDC}"
    )
    ...
    for idx, request in enumerate(driver.requests):
        # save the raw request data for later use
        har_data_raw.append(entry(request, idx))
        ...
        if content_type.startswith(("image/", "font/", "video/")):
            continue
        # if all filters are passed, add to HAR data
        har_data_filtered.append(entry(request, idx))

    with open(har_filename, "w", encoding="utf-8") as f:
        json.dump(har_data_filtered, f, indent=2)

    with open(har_filename_raw, "w", encoding="utf-8") as f:
        json.dump(har_data_raw, f, indent=2)

    print(
        f"{bcolors.OKGREEN}[SUCCESS]: Captured traffic saved to: {har_filename}{bcolors.ENDC}"
    )

    print(
        f"{bcolors.OKBLUE}[INFO]: Session recording complete. Keeping browser open for replay tests...{bcolors.ENDC}"
    )
    return har_filename, har_filename_raw, driver
